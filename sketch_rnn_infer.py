...
def load_dataset(data_dir, model_params, inference_mode=False):
  ...
  valid_strokes = {}
  test_strokes = {}
  train_strokes["combined"] = None

  for dataset in datasets:
    dataset_name = dataset[:dataset.index(".npz")]
    ...
    tf.logging.info('Loaded {}/{}/{} from {}'.format(
        ...
        dataset))
    train_strokes[dataset_name] = data['train']
    valid_strokes[dataset_name] = data['valid']
    test_strokes[dataset_name] = data['test']
    if train_strokes["combined"] is None:
      train_strokes["combined"] = data['train']
      valid_strokes["combined"] = data['valid']
      test_strokes["combined"] = data['test']
    else:
      train_strokes["combined"] = np.concatenate((train_strokes["combined"], data['train']))
      valid_strokes["combined"] = np.concatenate((valid_strokes["combined"], data['valid']))
      test_strokes["combined"] = np.concatenate((test_strokes["combined"], data['test']))

  all_strokes = np.concatenate((train_strokes["combined"], valid_strokes["combined"], test_strokes["combined"]))
  num_points = 0
  for stroke in all_strokes:
    num_points += len(stroke)
  avg_len = num_points / len(all_strokes)
  tf.logging.info('Dataset combined: {} ({}/{}/{}), avg len {}'.format(
      len(all_strokes), len(train_strokes["combined"]), len(valid_strokes["combined"]),
      len(test_strokes["combined"]), int(avg_len)))
  ...
  for dataset in datasets:
    dataset_name = dataset[:dataset.index(".npz")]
    train_set[dataset_name] = utils.DataLoader(
        train_strokes[dataset_name],
        model_params.batch_size,
        max_seq_length=model_params.max_seq_len,
        random_scale_factor=model_params.random_scale_factor,
        augment_stroke_prob=model_params.augment_stroke_prob)
    train_set[dataset_name].normalize(normalizing_scale_factor)

    valid_set[dataset_name] = utils.DataLoader(
        valid_strokes[dataset_name],
        eval_model_params.batch_size,
        max_seq_length=eval_model_params.max_seq_len,
        random_scale_factor=0.0,
        augment_stroke_prob=0.0)
    valid_set[dataset_name].normalize(normalizing_scale_factor)

    test_set[dataset_name] = utils.DataLoader(
        test_strokes[dataset_name],
        eval_model_params.batch_size,
        max_seq_length=eval_model_params.max_seq_len,
        random_scale_factor=0.0,
        augment_stroke_prob=0.0)
    test_set[dataset_name].normalize(normalizing_scale_factor)
