...
var sketch = function( p ) { 
  ...
  var math_away = function(obj_1, obj_2) {
    for(var i =0; i < obj_1.length; i ++){
      add_obj[i] = obj_1[i] + obj_2[i];
      sub_obj[i] = obj_1[i] - obj_2[i];
      time_obj[i] = obj_1[i] * obj_2[i];
      div_obj[i] = obj_1[i] / obj_2[i];
      eq1_obj[i] = obj_1[i]+obj_2[i]*5;
      eq2_obj[i] = Math.max(obj_1[i]*obj_2[i]);
      eq3_obj[i] = Math.min(obj_1[i]*obj_2[i]);
      eq4_obj[i] = Math.sqrt(obj_1[i]*obj_2[i]);
      eq5_obj[i] = Math.log(obj_1[i]*2+obj_2[i]);

    }
    var result = [];
    result[0] = obj_1; // "a"
    result[1] = add_obj; // "a+b"
    result[2] = sub_obj; // "a-b"
    // result[3] = time_obj; // "a*b"
    // result[4] = div_obj; // "a/b"
    // result[5] = eq1_obj; // "a+5b"
    // result[6] = eq2_obj; // "max(a,b)"
    // result[7] = eq3_obj; // "min(a,b)"
    // result[8] = eq4_obj; // "sqrt(a*b)"
    // result[9] = eq5_obj; // "log(2a+b)"
    result[3] = obj_2; // "b"
  ...
  var restart = function() {
    ...
    latent_grid = math_away(latent_1, latent_2);
  ...
  p.setup = function() {
    ...
    reset_1.position(5, screen_height/2-200);
    ...
    reset_2.position(screen_width-101, screen_height/2-200);
    reset_2.mousePressed(reset_2_event); // attach button listener

    // interp button
    interpolate_button = p.createButton('Math away!');
    // interpolate_button.position(screen_width/2-40, screen_height/2-image_spacing-27-20);
    interpolate_button.position(screen_width/2-40, screen_height/2-200);
